...
import {
  StyleSheet, Text, View, TouchableOpacity, Platform, ActivityIndicator, Dimensions, PickerIOS
} from 'react-native';
...
import { Avatar } from '../../components/ui/react-native-elements';
...
import {
  updateUsername, updateDob, updateGender, updateWeight, updateHeight, resetState,
  updateBmi, updateFetchedUrl, updateUserSocialImage, updateUserImageDetails, updateLeaderBoardData
...
class UserProfile extends React.Component {
  ...
  componentDidMount() {
    const {
      navigation, uName, updatedHeight, updatedWeight, updatedGender, updatedBmi, updatedDob
    } = this.props;
  ...
  onAddItem = async (selectedChoice) => {
    ...
    const stateHeightCondition = selectedChoice === 'height' ? heightList[tempHeight] : uHeight === 0 ? 122 : uHeight;
    const stateWeightCondition = selectedChoice === 'weight' ? weightList[tempWeight] : uWeight === 0 ? 24 : uWeight;
    await fetch(`${BASE_URL}/api/userData`, {
      ...
      .then(updatedBmi(CalculateBmi(selectedChoice === 'height' ? heightList[tempHeight] : uHeight,
  ...
  imageCondition = () => {
    ...
    if (fetchedUrl !== null && fetchedUrl !== '') {
      ...
      source = { uri: 'https://res.cloudinary.com/pratian-technologies/image/upload/v1560505157/Zul-Profile-Image/imageedit_8_5096704736.png' };
    }
    return source;
  };
  ...
  update = (imageDetails) => {
    ...
    if (count > 0) {
      this.setState({ spinner: false, count: 0 }, () => Toast.show({
  ...
  pickImageHandler = (mode) => {
    ...
    if (mode === "Camera") {
      ImagePickerCrop.openCamera({
        ...
        console.log(e);
        this.setState({ cameraModel: false }, () => Toast.show({
          text: e.message,
          duration: 2000,
          type: 'default'
        }))
  ...
  getWellnessAssessment = (data) => {
    ...
    fetch(requestUrl, {
      ...
      .then((responseJson) => {
        ...
        updatedCurrentAnswerId(null);
        navigation.navigate('UserWellnessAssessmentRouteAssessmentInfo', { title: data });
      })
      .catch((err) => { console.log('Network Error', err); });
  }
      ...
      .then((responseJsonOption) => {
        ...
        fetch(requestUrl, {
          ...
          .then((responseJson) => {
            ...
            navigation.navigate('UserWellnessAssessmentRouteAssessment', { title: data });
          })
          .catch((err) => { console.log('Network Error', err); });
      });
  }
  ...
  wellnessAssessment() {
    ...
    if (wellAssessment.drafted === 'completed') {
      ...
      //   type: 'default'
      // });
      this.getWellnesDraftedAssessment('Wellness', wellAssessment.id, 'completed');
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    return !(this.props.uBmi === nextProps.uBmi
  ...
  render() {
    //alert("count"+count++);
    const {
      isDialogVisible, height, weight, bmi, dob, spinner,
    ...
    const { navigation, uName, uGender, uHeight, uWeight, uDob, uBmi } = this.props;
    ...
    return (
      <View style={styles.container}>
        ...
        <Dialog.Container visible={modalVisible}>
          ...
          <RadioGroup selectedIndex={genderIndex} style={styles.radio} color="black" onSelect={(index1, value) => this.onSelect(index1, value)}>
        ...
        >
          ...
          <Dialog.Button label="Cancel" style={styles.dialogButtonWeight} onPress={this.toggleChangeDataModal('height', true)} />
          <Dialog.Button
            label="OK"
        ...
        >
          ...
          <Dialog.Button style={styles.dialogButtonWeight} label="Cancel" onPress={this.toggleChangeDataModal('weight', true)} />
          <Dialog.Button
            label="OK"
            style={styles.dialogButtonWeight}
            onPress={() => {
        ...
        <Dialog.Container visible={isDialogVisible} style={styles.dialogCon}>
          ...
          <Dialog.Button style={styles.dialogButton} label="Cancel" onPress={this.dailogVisible} />
          <Dialog.Button style={styles.dialogButton} label="Submit" onPress={this.submit} />
        </Dialog.Container>
        ...
        <Dialog.Container visible={this.state.cameraModel} onBackdropPress={() => this.cameraModel("Cancel")} style={{ borderRadius: 30 }}>
          ...
          <TouchableOpacity style={styles.cameraText} onPress={() => this.pickImageHandler("Camera")}>
            <Text style={styles.cameraTextStyle}>Take Photo...</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.cameraText} onPress={() => this.pickImageHandler("Gallary")}>
        ...
        <View style={{ flex: 1, flexDirection: 'column' }}>
          ...
          >
            <View style={styles.linearGradient2}>
              <TouchableOpacity onPress={() => navigation.navigate('OverviewRoute')} style={styles.linearGradient6}>
                <Icon style={styles.linearGradient3} name="arrow-left" size={25} color="#144e76" />
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.linearGradient8}
                onPress={() => navigation.navigate('UserWellnessAssessmentRouteReportIssue')}
              >
                <Icon1 style={styles.linearGradient4} name="support" size={20} color="#144e76" />
                <Text style={styles.linearGradient5}
                >
                  Support
                </Text>
              </TouchableOpacity>
            ...
            <View style={styles.linearGradient7}>
              ...
              <View style={styles.spinner}>
                {spinner
                  ? <ActivityIndicator animating={spinner} style={styles.spinner0} color="rgb(66, 159, 247)" size="large" />
                  : (
                    <Avatar
                      size={width * 0.33}
                      showEditButton
              ...
              <View style={{ flex: 1, alignItems: 'center' }}>
                ...
                {uDob
                  ? <Text style={{
                    fontFamily: 'System', fontSize: 20, color: '#144e76', paddingHorizontal: 5,
                    //                 elevation: 5,
            ...
            <View style={styles.spinner3}>
              <View style={styles.spinner4}>
                {uHeight
                  ...
                  : <TouchableOpacity style={styles.smallCards} onPress={this.toggleChangeDataModal('height')}>
                    <View style={styles.spinnerHeight}>
              ...
              <View style={styles.spinner4}>
                {uWeight
                  ...
                  <TouchableOpacity style={styles.smallCards} onPress={this.toggleChangeDataModal('weight')}>
          ...
          >
            ...
            >
              ...
              <TouchableOpacity
                ...
                onPress={() => navigation.navigate('UserWellnessAssessmentRouteLeaderBoard')}
              >
                <LinearGradient
                  start={{ x: 0, y: 0 }}
                  end={{ x: 1, y: 1 }}
            ...
            <View style={styles.keyButton3}>
              <TouchableOpacity
                style={styles.keyButton2}
                onPress={() => navigation.navigate('UserWellnessAssessmentRouteUserAccountControls')}
              >
                <LinearGradient
                  start={{ x: 0, y: 0 }}
                  end={{ x: 1, y: 1 }}
...
const styles = StyleSheet.create({
  ...
  height: { fontFamily: 'System', fontSize: 20, fontWeight: 'normal', color: '#000' },
  heightcm: { fontFamily: 'System', fontSize: 16, fontWeight: 'normal', color: '#000' },
  ...
  dialogTitleWeightStyle: { fontFamily: 'System', fontSize: 20, fontWeight: 'normal', color: '#000' },
  pickerIosHeight: { fontFamily: 'System', color: 'black', fontSize: 26 },
  dialogButtonWeight: { fontFamily: 'System', fontSize: 16 },
  dialogKg: { fontFamily: 'System', fontSize: 16, fontWeight: 'normal', alignItems: 'flex-end', color: '#000' },
  ...
  linearGradient5: { fontFamily: 'System', fontSize: 16, color: '#144e76', paddingHorizontal: 5 },
  linearGradient6: { flex: 1, flexDirection: 'row', alignItems: 'center' },
  linearGradient7: { flex: 3, flexDirection: 'row', alignItems: 'center' },
  linearGradient8: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end' },
  linearGradient9: { flex: 1, alignItems: 'center' },
  linearGradient10: { fontFamily: 'System', fontSize: 20, color: '#144e76', paddingHorizontal: 5 },
  spinner0: { paddingVertical: 35 },
  spinner: { flex: 1, alignItems: 'center', borderColor: '#f2f2f2', borderWidth: 1, borderRadius: 100 },
  spinner1: { flex: 0.8, justifyContent: 'center', alignItems: 'center' },
  spinner2: { fontFamily: 'System', fontSize: 25, color: '#144e76' },
  spinner3: { flex: 1, flexDirection: 'row', borderBottomWidth: 2, borderColor: '#f2f2f2' },
  ...
  smallCards1: { fontFamily: 'System', fontSize: 16, color: '#144e76', paddingLeft: 5, },
  spinnerHeight2: { fontFamily: 'System', fontSize: 16, color: '#000', paddingLeft: 5 },
  ...
  keyButton: { flex: 6, flexDirection: 'column', padding: 10, backgroundColor: '#fff' },
  keyButton1: { flex: 1, flexDirection: 'row', borderBottomWidth: 0.5, borderColor: '#D3D3D3' },
  keyButton2: { flex: 1, justifyContent: 'center', alignItems: 'center', borderRightWidth: 1, borderColor: '#D3D3D3' },
